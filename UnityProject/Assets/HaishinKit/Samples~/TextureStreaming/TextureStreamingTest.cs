...
namespace HaishinKit.Samples
{
    ...
    public class TextureStreamingTest : MonoBehaviour
    {
        ...
        {
            // RuntimeInitializeOnLoadMethod により自動生成されるが、念のため確認
            _manager = HaishinKitManager.Instance;
        }

        private void SetupEventHandlers()
        {
            // enum ベースのステータスイベント（推奨）
            _manager.OnStreamingStatusChanged += OnStreamingStatusChanged;
            // 個別イベント
        ...
        private void OnStreamingStatusChanged(StreamingStatus status)
        {
            if (enableDebugLog)
            {
                Debug.Log($"[TextureStreaming] Status: {status}");
            }
        }
        ...
        private void SendVideoFrame()
        {
            ...
            // Android: RenderTexture を直接渡す（バックエンドが ReadPixels / AsyncGPU / NativeTexture を選択）
            _manager.SendVideoFrame(_renderTexture);
#else
            // iOS/macOS: Metal テクスチャポインタを渡す
        ...
        private void DrawUI()
        {
            ...
            // Status (enum)
            GUI.Label(new Rect(margin, y, w, h), $"Texture Streaming - {_manager?.Status}");
            y += h;

            // Stats
            if (_manager != null && _isPublishing)
            {
                var stats = _manager.Stats;
                GUI.Label(new Rect(margin, y, w, h * 0.5f), $"FPS: {stats.CurrentFps:F1} | Video: {stats.VideoFramesSent} | Audio: {stats.AudioFramesSent} | Dropped: {stats.DroppedFrames}");
                y += h * 0.6f;
            }
